...
<!-- ### Gestione caratteri non correttamente renderizzati -->

<!-- Sono codificati in HEX, in ogni caso non dovrebbe essere necessaria la conversione usando un charset di tipo UTF-8 durante l'importazione su un software sql. -->

<!-- In ogni caso, vanno verificate le corrispondenze con il database sulle stazioni. -->
...
<!-- > s_r -->
<!-- [1] f3 -->
<!-- > t <- rawToChar(as.raw(strtoi(s_r, 16L))) -->
<!-- > t -->
<!-- [1] "ó" -->
...
<!-- Poiché per lo scopo di questa applicazione è sufficiente un database concernente i dati più recenti riguardo le città, verranno rimosse le quantità temporali in modo tale da avere un valore per ogni città e rendere più agevoli le operazioni di calcolo sul database. -->

<!-- *Si noti che la successione temporale è del tutto irrilevante per i fini dell'applicazione, che solamente valuta il valore più recente.* -->

# Operazioni sul database delle stazioni
...
<!--  normalised_code VARCHAR(30),        -->
<!--  iata_airport_code VARCHAR(3) -->
